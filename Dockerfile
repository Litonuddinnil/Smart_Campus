...
# Dockerfile, the Space, and the local docs all agree.
EXPOSE 7860
ENV PORT=7860
ENV HOST=0.0.0.0
...
# `docker run -e ...` locally, or as Space Secrets on Hugging Face.
...
# would still accept.
CMD ["gunicorn", "--bind", "0.0.0.0:7860", "--workers", "1", "--threads", "4", "--timeout", "35", "app:app"]
